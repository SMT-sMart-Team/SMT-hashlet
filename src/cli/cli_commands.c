...
void set_defaults (struct arguments *args)
{
  ...
  args->verbose = 0;
  args->output_file = "-";
  args->input_file = "/root/SMT/.smt_words.txt";
...
int cli_mac (int fd, struct arguments *args)
{
          ...
          if (rsp.status)
            {
              ...
              result = cli_verify_mac1(2,challenge,rsp.mac);
  ...
  printf ("%s\n", NO_GCRYPT);
#endif
  if(result == HASHLET_COMMAND_SUCCESS)
...
int cli_verify_mac (int fd, struct arguments *args)
{
  int result = HASHLET_COMMAND_FAIL;
  assert (NULL != args);
  printf("chllg %s \n rsp %s \n slot %d \n",args->challenge,args->challenge_rsp,args->key_slot);
#if 1
  const char* key;
  struct octet_buffer challenge;
...
int cli_verify_mac1(int keyslot,struct octet_buffer chllg,struct octet_buffer chllg_rsp)
{
	int result = HASHLET_COMMAND_FAIL;
	const char* key;
	struct octet_buffer challenge;
	...
	if(chllg.ptr == NULL || chllg_rsp.ptr == NULL)
	{
	...
	if ((key = get_key_from_store(keyslot)) == NULL)
		fprintf(stderr, "%s\n", "Invalid file or file failed to parse");
	else
	{
		challenge = chllg;
		challenge_rsp = chllg_rsp;

		key_buf = ascii_hex_2_bin(key, SIZE_OF_256_BITS_ASCII);
		if (challenge.ptr != NULL && challenge_rsp.ptr != NULL && key_buf.ptr
				!= NULL)
		{
			if (verify_hash_defaults(challenge, challenge_rsp, key_buf,
					keyslot)) {
